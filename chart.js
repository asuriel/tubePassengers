...
var xScale = d3.scale.linear().domain([0,53]).range([0, width]), 
    yScale = d3.scale.linear().domain([0,9]).range([height, 0]),
...
var xAxis = d3.svg.axis().orient("bottom").scale(xScale).ticks(20, d3.format(",d")),
    yAxis = d3.svg.axis().scale(yScale).orient("left").ticks(9, d3.format("s")).tickValues(["A","B","C","D","E","F","G","H",]);
